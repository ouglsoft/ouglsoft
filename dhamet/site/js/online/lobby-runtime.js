...
(function () {
  ...
  const Online = {
    ...
    _dispatchUnifiedAppPulseNow: async function (force, reason) {
          try {
            ...
            if (!window.DhametGameRoomClient || typeof window.DhametGameRoomClient.commitAppPulse !== "function") {
              throw new Error("game-room-client-missing");
            }
            const result = await window.DhametGameRoomClient.commitAppPulse(payload);
    ...
    _markPlayerBusyWithRoom: async function (gameId, options) {
          const gid = String(gameId || "").trim();
    ...
    _markBusyIfActivePlayerRoom: async function (options) {
          ...
          if (!gid) return false;
          return await this._markPlayerBusyWithRoom(gid, options);
        },
    ...
    _validateInviteBeforeAccept: async function (inv) {
          try {
    ...
    _acceptInviteLobby: async function (inv, inviteRef) {
          try {
            ...
            const validated = await this._validateInviteBeforeAccept(inv);
  ...
  window.__ZAMAT_ONLINE_SHARED__ = Object.freeze({
    ASSET_PREFIX,
