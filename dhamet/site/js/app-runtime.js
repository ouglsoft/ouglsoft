...
function persistNickIcon(session){
  try {
    const nickname = String(session && session.nickname || "").trim();
    const kind = String(session && session.kind || "").trim();
    const uid = String(session && session.uid || "").trim();
    const generatedGuestNick = !!(kind === "guest" && nickname && uid && nickname === `Guest ${uid.slice(-4)}`);

    if (kind === "registered" && nickname) {
      try { sessionStorage.setItem(NICK_LS_KEY, nickname); } catch {}
      ...
      } else if (nickname && !generatedGuestNick && !explicit) {
        try { sessionStorage.setItem(NICK_LS_KEY, nickname); } catch {}
        try { sessionStorage.setItem(NICK_EXPLICIT_KEY, "1"); } catch {}
      }
    }
...
(function(){
  function __z_init_pages_shell(){
    (function () {
      ...
      var AppPref = globalThis.AppPref || (globalThis.AppPref = Object.freeze({
        getLang: function () {
          try {
