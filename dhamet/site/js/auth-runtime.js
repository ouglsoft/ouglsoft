...
(function () {
  ...
  function reauthenticateGoogle() {
    if (!cachedUser || cachedUser.isAnonymous) return Promise.reject(makeError('auth/unauthenticated', 401));
    var returnPath = '/pages/dashboard.html';
    try { sessionStorage.setItem('dhamet.google.reauth.pending.v1', '1'); } catch (_) {}
    location.href = api('/dhamet/api/auth/google/start?reauth=1&returnPath=' + encodeURIComponent(returnPath));
    return new Promise(function () {});
  }
